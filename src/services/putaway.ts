...
export const fetchPutawayMixed = async (
  ...
  // const url = `/maximo/oslc/os/WMS_MXWOISSUE?lean=1&oslc.select=*&oslc.where=siteid="${siteid}" and invuse.usetype="MIXED" and invuse.status!="COMPLETE" and wonum="${wonum}" and invuse.invuseline.invuselineid="${invuselineid}"&oslc.pageSize=${pageSize}`;
  const url = `/maximo/oslc/os/WMS_MXWOISSUE?lean=1&oslc.select=wonum,description,statusdate&oslc.where=siteid="${siteid}" and invuse.usetype="MIXED" and invuse.status!="COMPLETE" and wonum="${wonum}" and invuse.invuseline.invuselineid="${invuselineid}"&oslc.pageSize=${pageSize}`;
...
export const fetchPutawayMixedSingle = async (
  wonum: string = '',
  invuselineid: string = '',
  pageSize: number = 100,
) => {
